	...
	public class StatusDisplayerAutoSetup : MonoBehaviour {
		...
		void RegisterIfSomethingVoid(UnityEventBase poller, UnityAction f) {
				...
				if (this._clean_empty_no_target_events && count > 0) {
					//poller.PollEvent.RemoveAllListeners(); // Only non-persistant listeners.
					for (var i = 0; i < count; i++) {
						if (poller.GetPersistentTarget(i) == null
						    || poller.GetPersistentMethodName(i) == null) {
		...
		void Start () {
			...
			if (this._environment) {
				this.RegisterIfSomething(this._enviroment_text,
						this._environment.IdentifierString);
				this.RegisterIfSomething(this._enviroment_frame,
						this._environment.FrameString);
				this.RegisterIfSomething(this._terminated,
						this._environment.TerminatedBoolean);
			}
			...
			if (this._evaluation_function) {
				this.RegisterIfSomething(this._signal,
						this._evaluation_function.SignalString);
			}
			...
			if (this._manager) {
				if (this._status_text) {
					this.RegisterIfSomething(this._status_text,
							this._manager.StatusString);
				}


				if(this._testing_toggle) {
					this.RegisterIfSomethingProperty(this._testing_toggle.onValueChanged,
							this._manager.SetTesting);
				}
			}

			if(this._reset_button) {			
				this.RegisterIfSomethingVoid(this._reset_button.onClick,
						this._manager.ResetAllEnvironments);
			}
		}
	}
}
