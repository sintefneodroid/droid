...
namespace droid.Runtime.Prototyping.Sensors.Rays {
  ...
  public class SweepingLidarSensor : Sensor,
    ...
    Space1 _sweeping_space =
        new Space1 {Min = -180, Max = 180, DecimalGranularity = 5, NormalisedBool = true};
    ...
    public override void Tick() {
      ...
      if (!this.loop) {
        a = this._sweeping_space.Denormalise01(Mathf.Cos(a) * .5f + .5f);
      }
    ...
    public override void UpdateObservation() {
      if (Physics.Raycast(this.transform.position + this._ray_space.Min * this.current_direction,
                          ...
                          out this._hit,
                          this._ray_space.Max)) {
        this.ObservationValue = this._ray_space.ClipNormaliseRound(this._hit.distance);
      } else {
