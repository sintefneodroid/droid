...
Shader "Neodroid/Experimental/MultiMode"
{
    Properties
    {
        _MainTex ("", 2D) = "white" {}
...
#include "UnityCG.cginc"
        ...
        float4 Output(float depth01, float3 normal)
        {
            ...
            {
                // DepthCompressed
                float linear_z_from_near = Linear01FromEyeToLinear01FromNear(depth01);
                float k = 0.25; // compression factor
                return pow(linear_z_from_near, k);
            }
            else if (_OutputMode == 3)
            {
                // DepthMultichannel
                float low_bits = frac(depth01 * 256);
                float high_bits = depth01 - low_bits / 256;
                return float4(low_bits, high_bits, depth01, 1);
            }
