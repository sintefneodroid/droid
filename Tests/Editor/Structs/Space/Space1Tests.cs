...
namespace droid.Tests.Editor.Structs.Space {
  ...
  public class Space1Tests {
    ...
    public float TestDenormalise010(float v, float min_value, float max_value) {
      var space = new Space1 {Min = min_value, Max = max_value, Normalised = NormalisationEnum.Zero_one_, DecimalGranularity = 1};
    ...
    public void TestDenormalise010Throws(float v) {
      var space = new Space1 {Min = 0, Max = 10, Normalised = NormalisationEnum.Zero_one_};

      Assert.That(() => space.Reproject(v : v), Throws.TypeOf<ArgumentException>());
    }
    ...
    public void TestNormalise010Throws(float v) {
      ...
      Assert.That(() => space.Project(v : v), Throws.TypeOf<ArgumentException>());
    }
    ...
    public float TestNormalise010(float v, float min_value, float max_value) {
      var space = new Space1 {Min = min_value, Max = max_value,DecimalGranularity = 1, Normalised = NormalisationEnum.Zero_one_};

      //Assert.AreEqual(expected, result, tolerance);
      return  space.Round(space.Project(v : v));
    }
    ...
    public void TestNormaliseMinus11() {
      var space = new Space1 {Min = -1, Max = 1, Normalised = NormalisationEnum.Zero_one_};
    ...
    public void TestDenormaliseMinus11() {
      var space = new Space1 {Min = -1f, Max = 1f, Normalised = NormalisationEnum.Zero_one_,DecimalGranularity = 1};
